use std::path::PathBuf;

use cfg_if::cfg_if;

#[derive(Debug, Clone)]
pub struct Paths {
    pub nvim_config: PathBuf,
    pub nvim_data: PathBuf,
}

#[cfg(windows)]
const NVIM_DATA_DIR: &str = "nvim-data";

#[cfg(unix)]
const NVIM_DATA_DIR: &str = "nvim";

impl Paths {
    pub fn new() -> Self {
        cfg_if! {
            if #[cfg(test)] {
                let local_config_dir = std::env::current_dir().unwrap().join("test-assets");
                let local_data_dir = std::env::current_dir().unwrap().join("test-assets");
            } else if #[cfg(target_os = "macos")] {
                let local_config_dir = dirs::home_dir().unwrap().join(".config");
                let local_data_dir = dirs::home_dir() .unwrap().join(".local").join("share");
            } else {
                let local_config_dir = dirs::config_local_dir().unwrap();
                let local_data_dir = dirs::data_local_dir().unwrap();
            }
        }

        #[cfg(not(test))]
        return Self {
            nvim_config: local_config_dir.join("nvim"),
            nvim_data: local_data_dir.join(NVIM_DATA_DIR),
        };

        #[cfg(test)]
        Self {
            nvim_config: local_config_dir,
            nvim_data: local_data_dir,
        }
    }
}
