...
mod lock_specific {
    ...
    #[test]
    fn test_installed() {
        let dir = OUTPUTS.join("test-lock-specific-installed");
        setup(&dir);
    ...
    fn test_not_installed() {
        let dir = OUTPUTS.join("test-lock-specific-not-installed");
        setup(&dir);
        ...
        let mut cmd = Command::cargo_bin("ts-parsers").unwrap();
        cmd.env("TS_PARSERS_DATA", &dir);
        cmd.args(["install", "rust", "blueprint"]);
        cmd.assert().success();
        validate_state(&dir);

        let mut cmd = Command::cargo_bin("ts-parsers").unwrap();
        cmd.env("TS_PARSERS_DATA", &dir);
        cmd.args(["lock", "rust", "lua", "blueprint"]);
...
mod lock_wanted {
    ...
    #[test]
    fn test_installed() {
        let dir = OUTPUTS.join("test-lock-wanted-installed");
        setup(&dir);
    ...
    #[test]
    fn test_not_installed() {
        let dir = OUTPUTS.join("test-lock-wanted-not-installed");
        setup(&dir);
